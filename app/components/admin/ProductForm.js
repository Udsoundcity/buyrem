              ...
              <>
                <div className="admin-form-card">
                  ...
                  <div className="admin-grid-2">
                    ...
                    <Field label="Badge Text" hint="e.g. Bestseller, New, Popular (optional)">
                      <input className="admin-input" value={form.badge}
                        onChange={e => set("badge",e.target.value)} placeholder="Bestseller" />
                    </Field>
                  </div>
                  ...
                  <Field label="Description" required>
                    <textarea className={`admin-textarea${errors.description?" error":""}`}
                ...
                <div className="admin-form-card">
                  ...
                  <p className="admin-input-hint" style={{marginBottom:14}}>
                    Shown on the homepage card and products listing. Upload from your device or paste a URL.
                  </p>
                  {errors.thumbnail && (
                    <p style={{color:"#FCA5A5",fontSize:12,marginBottom:10}}>⚠️ {errors.thumbnail}</p>
                  )}
                  <ImageUpload
                    value={form.thumbnail}
                    onChange={url => { set("thumbnail",url); setErrors(p=>({...p,thumbnail:""})); }}
                  />
                </div>
              ...
              <div className="admin-form-card">
                <div className="admin-form-card-title">
                  ...
                  <span style={{fontSize:12,fontWeight:400,color:"var(--a-muted)",marginLeft:8}}>
                    4 photos shown on the product detail page
                  </span>
                </div>
                <p className="admin-input-hint" style={{marginBottom:20}}>
                  Upload from your device or paste an image URL. For each image, also set an alt text and a label.
                </p>
                ...
                {form.images.map((img,i) => (
                  <div key={i} className="admin-array-item">
                    ...
                    <div className="admin-grid-2">
                      ...
                      <ImageUpload
                        value={img.src}
                        onChange={url => set(`images.${i}.src`, url)}
                      />
                      {/* Meta */}
                      <div style={{display:"flex",flexDirection:"column",gap:12}}>
                        <Field label="Alt Text" hint="Describes the image (accessibility)">
                          <input className="admin-input" value={img.alt}
                            onChange={e => set(`images.${i}.alt`,e.target.value)}
                            placeholder="e.g. Serum bottle front view" />
                        </Field>
                        <Field label="Label" hint="Caption shown under thumbnail">
                          <input className="admin-input" value={img.label}
                            onChange={e => set(`images.${i}.label`,e.target.value)}
                            placeholder="e.g. Product Front" />
                        </Field>
                      </div>
                    </div>
                  </div>
                ...
                <div className="admin-form-card">
                  ...
                  <Field label="Problem Description">
                    <textarea className="admin-textarea" value={form.problem.body}
                      ...
                      placeholder="Describe the pain point your customer experiences..." />
                  </Field>
                </div>
                    ...
                    <div key={i} className="admin-array-item">
                      ...
                      <div className="admin-grid-3">
                        <Field label="Icon (emoji)">
                          <input className="admin-input" value={s.icon}
                            onChange={e => set(`solutions.${i}.icon`,e.target.value)} placeholder="🌟" />
                        </Field>
                        <Field label="Title">
                          <input className="admin-input" value={s.title}
                            onChange={e => set(`solutions.${i}.title`,e.target.value)} placeholder="Fades dark spots" />
                        </Field>
                        <Field label="Description">
                          <input className="admin-input" value={s.body}
                            onChange={e => set(`solutions.${i}.body`,e.target.value)} placeholder="How this helps..." />
                        </Field>
                      </div>
                    </div>
                  ))}
                    ...
                    {form.features.map((f,i) => (
                      <div key={i} className="admin-feature-row">
                        ...
                        <input className="admin-input" value={f}
                          onChange={e => set(`features.${i}`,e.target.value)} placeholder={`Feature ${i+1}…`} />
                      </div>
                    ))}
                  </div>
                </div>
                      ...
                      <div className="admin-grid-2">
                        <Field label="Customer Name">
                          <input className="admin-input" value={t.name}
                            onChange={e => set(`testimonials.${i}.name`,e.target.value)} placeholder="Amaka O." />
                        </Field>
                        <Field label="Location">
                          <input className="admin-input" value={t.location}
                            onChange={e => set(`testimonials.${i}.location`,e.target.value)} placeholder="Surulere, Lagos" />
                        </Field>
                      </div>
                      <Field label="Rating (1–5)">
                        <select className="admin-select" value={t.rating}
                          onChange={e => set(`testimonials.${i}.rating`,Number(e.target.value))}>
                          {[5,4,3,2,1].map(r => <option key={r} value={r}>{r} ⭐</option>)}
                        </select>
                      </Field>
                      <Field label="Review Text">
                        <textarea className="admin-textarea" value={t.text}
                          onChange={e => set(`testimonials.${i}.text`,e.target.value)}
                          rows={2} placeholder="What the customer said..." />
                      </Field>
                    </div>
                  ))}
                </div>
                  ...
                  {form.faq.map((f,i) => (
                    <div key={i} className="admin-array-item">
                      ...
                      <Field label="Question">
                        <input className="admin-input" value={f.q}
                          onChange={e => set(`faq.${i}.q`,e.target.value)} placeholder="e.g. How soon will I see results?" />
                      </Field>
                      <Field label="Answer">
                        <textarea className="admin-textarea" value={f.a}
                          onChange={e => set(`faq.${i}.a`,e.target.value)} rows={2} placeholder="Clear, concise answer." />
                      </Field>
                    </div>
                  ))}
                </div>
