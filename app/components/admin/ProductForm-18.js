...
const TABS=["Basic Info","Images & Video","Sales Copy","Reviews & FAQ","Stories"];
      ...
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <button type="button" onClick={()=>onChange(!checked)}
          style={{width:44,height:24,borderRadius:100,border:"none",cursor:"pointer",
            background:checked?"#22C55E":"#334155",position:"relative",transition:"background 0.2s",flexShrink:0}}>
          ...
          <div className="admin-form-page">
            ...
            {tab===4&&(
              <>
                {/* Top Story */}
                <div className="admin-form-card">
                  <div className="admin-form-card-title">📢 Top Story Section <span style={{fontSize:12,fontWeight:400,color:"var(--a-muted)"}}>— Optional</span></div>
                  <p className="admin-input-hint" style={{marginBottom:16}}>Displayed immediately below the navbar, before the product hero. Only visible when enabled and content is provided.</p>
                  <Toggle
                    label="Enable Top Story"
                    checked={!!form.topStory?.enabled}
                    onChange={v=>set("topStory.enabled",v)}
                    hint="Toggle off to completely hide this section with no empty space."
                  />
                    ...
                    <>
                      <Field label="Story Text" hint="Short attention-grabbing line displayed above the story image.">
                        <textarea className="admin-textarea" rows={3}
                          value={form.topStory?.text||""}
                          onChange={e=>set("topStory.text",e.target.value)}
                          placeholder="e.g. Thousands of Nigerians are already seeing results..."/>
                      </Field>
                      ...
                      <ImageUpload
                        value={form.topStory?.image||""}
                        onChange={url=>set("topStory.image",url)}
                        hint="Wide banner image shown below the story text."
                      />
                    </>
                  )}
                </div>
                ...
                {/* Customer Story */}
                <div className="admin-form-card">
                  <div className="admin-form-card-title">💬 Customer Story Section <span style={{fontSize:12,fontWeight:400,color:"var(--a-muted)"}}>— Optional</span></div>
                  <p className="admin-input-hint" style={{marginBottom:16}}>A longer customer experience section displayed after the product illustration video. Only visible when enabled and content is provided.</p>
                  <Toggle
                    label="Enable Customer Story"
                    checked={!!form.customerStory?.enabled}
                    onChange={v=>set("customerStory.enabled",v)}
                    hint="Toggle off to completely hide this section with no empty space."
                  />
                  {form.customerStory?.enabled&&(
                    <>
                      <Field label="Headline" hint="e.g. What one of our customers had to say">
                        <input className="admin-input" value={form.customerStory?.headline||""}
                          onChange={e=>set("customerStory.headline",e.target.value)}
                          placeholder="What one of our customers had to say"/>
                      </Field>
                      <Field label="Story Content" hint="Longer customer story — as long as needed. Supports line breaks.">
                        <textarea className="admin-textarea" rows={6}
                          value={form.customerStory?.content||""}
                          onChange={e=>set("customerStory.content",e.target.value)}
                          placeholder="I had been struggling with my skin for years before I found this serum..."/>
                      </Field>
                      <div className="admin-form-card-title" style={{fontSize:13,marginBottom:12,marginTop:4}}>Story Image <span style={{fontWeight:400,color:"var(--a-muted)"}}>— Optional</span></div>
                      <ImageUpload
                        value={form.customerStory?.image||""}
                        onChange={url=>set("customerStory.image",url)}
                        hint="Customer photo, before/after, or any supporting image."
                      />
                      <div className="admin-grid-2" style={{marginTop:16}}>
                        <Field label="CTA Button Text" hint="Text shown on the action button">
                          <input className="admin-input" value={form.customerStory?.ctaText||""}
                            onChange={e=>set("customerStory.ctaText",e.target.value)}
                            placeholder="Order Now"/>
                        </Field>
                        <Field label="CTA Action" hint="What happens when the button is clicked">
                          <select className="admin-select" value={form.customerStory?.ctaType||"whatsapp"}
                            onChange={e=>set("customerStory.ctaType",e.target.value)}>
                            <option value="whatsapp">💬 WhatsApp Order (opens order form)</option>
                            <option value="form">📋 Scroll to Order Form</option>
                          </select>
                        </Field>
                      </div>
                      <div style={{marginTop:8,padding:"10px 14px",background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:8,fontSize:13,color:"#94A3B8"}}>
                        {form.customerStory?.ctaType==="form"
                          ? "📋 Button will scroll to the embedded order form at the bottom of the page."
                          : "💬 Button will open the WhatsApp order form."}
                      </div>
                    </>
                  )}
                </div>
