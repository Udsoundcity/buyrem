...
// GET /api/admin/form-settings/[id]
export async function GET(_, { params }) {
  try {
    const { data, error } = await db()
      .from("settings").select("*").eq("id", params.id).single();
...
// PUT /api/admin/form-settings/[id]
export async function PUT(req, { params }) {
  try {
    ...
    if (error) {
      if (error.code === "23505")
        return NextResponse.json({ error:"Another form already uses this slug." }, { status:409 });
      throw error;
    }
    return NextResponse.json(data);
  } catch (e) {
...
// DELETE /api/admin/form-settings/[id]
export async function DELETE(_, { params }) {
  try {
    const { error } = await db().from("settings").delete().eq("id", params.id);
    if (error) throw error;
